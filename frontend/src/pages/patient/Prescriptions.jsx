...
export const Prescriptions = () => {
  ...
  const handleAddMedication = async () => {
    if (!addForm.medication_name || !addForm.dosage || !addForm.frequency) {
      showToast(t("fillMedFields"), "error");
      return;
    }
    setAddSaving(true);
    try {
      await prescriptionsAPI.addSelf(addForm);
      showToast(t("medicationAdded"));
  ...
  const handleDownload = (rx) => {
    ...
    showToast(`${t("downloaded")} ${rx.name}`);
  };
    ...
    <>
      ...
      <div className="page-responsive" style={{ background: "var(--bg)", minHeight: "100vh", padding: "40px 48px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ marginBottom: "36px", animation: "fadeUp 0.5s ease both" }}>
          <p style={{ color: "#10b981", fontSize: "12px", fontWeight: "600", letterSpacing: "2px", textTransform: "uppercase", margin: "0 0 6px 0" }}>{t("healthcare")}</p>
          <h1 style={{ color: "var(--text)", fontSize: "32px", fontWeight: "700", margin: 0, fontFamily: "'Playfair Display', serif", letterSpacing: "-0.5px" }}>{t("prescriptions")}</h1>
        </div>

        {/* Stats Row */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px", marginBottom: "32px", animation: "fadeUp 0.5s ease 0.08s both" }}>
          {[
            { label: t("activeMedicines"), value: prescriptions.filter(r => r.active).length, color: "#3b82f6" },
            { label: t("refillsNeeded"), value: prescriptions.filter(r => r.source !== "self" && r.refillsAllowed > 0 && r.refillsLeft === 0 && r.active).length, color: "#f59e0b" },
            { label: t("totalPrescriptions"), value: prescriptions.length, color: "#10b981" },
          ].map((s, i) => (
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "28px", flexWrap: "wrap", animation: "fadeUp 0.5s ease 0.12s both" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            {[["all", t("all")], ["active", t("activeStatus")], ["inactive", t("inactive")], ["refill", t("needsRefill")]].map(([val, label]) => (
        ...
        {showAddModal && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(4px)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center" }}
            ...
            <div className="modal-responsive" style={{ background: "var(--bg-3)", border: "1px solid var(--border-solid)", borderRadius: "16px", padding: "32px", width: "460px", maxWidth: "90vw", animation: "fadeUp 0.3s ease" }}>
              <h2 style={{ color: "var(--text)", fontSize: "20px", fontWeight: "700", margin: "0 0 6px 0" }}>{t("addYourMedication")}</h2>
              <p style={{ color: "var(--text-subtle)", fontSize: "13px", margin: "0 0 24px 0" }}>{t("addMedDesc")}</p>
              ...
              <div style={{ display: "flex", gap: "12px", marginTop: "24px" }}>
                ...
                <button className="rx-btn" onClick={handleAddMedication} disabled={addSaving}
                  ...
                  {addSaving ? t("adding") : t("addMedication")}
                </button>
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          {loading && (
            <div style={{ textAlign: "center", color: "var(--text-subtle)", padding: "60px", fontSize: "16px" }}>{t("loadingPrescriptions")}</div>
          )}
          {filtered.map((rx, i) => {
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: "20px" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px", flexWrap: "wrap" }}>
                      ...
                      {isSelf && <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", background: "rgba(139,92,246,0.12)", color: "#a78bfa", border: "1px solid rgba(139,92,246,0.25)" }}>{t("selfAdded")}</span>}
                      {!rx.active && <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", background: "rgba(71,85,105,0.2)", color: "var(--text-subtle)", border: "1px solid var(--border-solid)" }}>{t("inactive")}</span>}
                      {taken && <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "700", background: "rgba(16,185,129,0.12)", color: "#10b981", border: "1px solid rgba(16,185,129,0.3)" }}>✓ {t("takenToday")}</span>}
                    </div>
                    <div style={{ color: "var(--text-subtle)", fontSize: "13px", marginBottom: "16px" }}>
                      {rx.frequency}{isSelf ? ` · ${t("addedOn")} ${rx.prescribed}` : ` · ${t("prescribedBy")} ${rx.doctor} ${t("on")} ${rx.prescribed}`}
                    </div>

                    {/* Refills — only for provider-prescribed with refills enabled */}
                    {!isSelf && rx.refillsAllowed > 0 && (
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "4px" }}>
                        <span style={{ color: "var(--text-subtle)", fontSize: "13px" }}>{t("refillsRemaining:")}</span>
                        <span style={{ color: rx.refillsLeft === 0 ? "#ef4444" : "#10b981", fontWeight: "700", fontSize: "14px" }}>
                          {rx.refillsLeft === 0 ? t("noneRefillNeeded") : rx.refillsLeft}
                        </span>
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px", flexShrink: 0 }}>
                      ...
                      {!isSelf && rx.refillsAllowed > 0 && rx.refillsLeft === 0 && (
                        <button className="rx-btn" onClick={() => handleRefill(rx.id)}
                          ...
                          {refillRequested[rx.id] ? "✓ " + t("requestedStatus") : t("requestRefill")}
                        </button>
                      )}
